...
import { DOCUMENT_LIVE_SUPPLIERS_CATALOG } from '../src/data/liveSuppliersCatalog';
import { DOCUMENT_ITEM_MASTER_CATALOG } from '../src/data/masterItemsCatalog';
...
async function main() {
  console.log(`\n======================================================`);
  console.log(`🚀 REBOOT ERP -> SUPABASE CLOUD LIVE TS SEEDER`);
  ...
  // 4. Seed Suppliers
  console.log(`🏭 Seeding ${DOCUMENT_LIVE_SUPPLIERS_CATALOG.length} Suppliers into Supabase Cloud...`);
  const batchSize = 100;
  for (let i = 0; i < DOCUMENT_LIVE_SUPPLIERS_CATALOG.length; i += batchSize) {
    const batch = DOCUMENT_LIVE_SUPPLIERS_CATALOG.slice(i, i + batchSize).map(s => ({
      id: s.id || `SUP-${s.code}`,
      code: s.code,
      name: s.name,
    ...
    if (error) {
      console.error(`  Supplier Batch ${i} error:`, error.message);
    } else {
      console.log(`  ✓ Seeded suppliers ${i + 1} to ${Math.min(i + batchSize, DOCUMENT_LIVE_SUPPLIERS_CATALOG.length)}`);
    }
  }
  console.log(`✅ All ${DOCUMENT_LIVE_SUPPLIERS_CATALOG.length} Suppliers seeded into Supabase Cloud!`);

  // 5. Seed Master Items
  console.log(`📦 Seeding ${DOCUMENT_ITEM_MASTER_CATALOG.length} Master Items into Supabase Cloud...`);
  for (let i = 0; i < DOCUMENT_ITEM_MASTER_CATALOG.length; i += batchSize) {
    const batch = DOCUMENT_ITEM_MASTER_CATALOG.slice(i, i + batchSize).map(item => ({
      code: item.code,
      name: item.name,
      ...
      stock: parseFloat(String(item.stock || 0)) || 0,
      min_stock: parseFloat(String(item.minStock || 0)) || 0,
      max_stock: parseFloat(String(item.maxStock || 0)) || 0,
      reorder_point: parseFloat(String(item.reorderPoint || 0)) || 0,
      cost: parseFloat(String(item.cost || 0)) || 0,
      selling_price: parseFloat(String(item.sellingPrice || 0)) || 0,
      approval: 'approved',
    ...
    if (error) {
      console.error(`  Item Batch ${i} error:`, error.message);
    } else {
      console.log(`  ✓ Seeded items ${i + 1} to ${Math.min(i + batchSize, DOCUMENT_ITEM_MASTER_CATALOG.length)}`);
    }
  }
  console.log(`✅ All ${DOCUMENT_ITEM_MASTER_CATALOG.length} Master Items seeded into Supabase Cloud!`);
