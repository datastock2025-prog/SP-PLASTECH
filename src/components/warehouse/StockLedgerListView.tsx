...
export const StockLedgerListView: React.FC<Props> = ({
  ...
  return (
    <div className="space-y-4 pb-12">
      ...
      {/* Row Click / Modal: LEDGER ITEMS & LOT PROVENANCE FOR SELECTED ITEM (Task-2 & Task-4) */}
      {selectedItemForLots && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4 backdrop-blur-xs">
          <div className="bg-white rounded-2xl max-w-5xl w-full p-6 space-y-4 shadow-2xl border border-slate-200 animate-fade-in max-h-[92vh] overflow-y-auto">
            {/* Modal Header */}
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              <div>
                <div className="flex items-center gap-2">
                  <span className="font-mono text-sm font-bold text-[#0F8B8D]">{selectedItemForLots.sku}</span>
                  <WarehouseStatusBadge status={selectedItemForLots.status} size="xs" />
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-blue-50 text-blue-700 border border-blue-200">
                    {resolveStoreType(selectedItemForLots)} STORE
                  </span>
                </div>
                <h3 className="font-bold font-['Space_Grotesk'] text-lg text-[#14213D] mt-0.5">
                  {selectedItemForLots.name} &mdash; Item Lot Ledger Grid
                </h3>
              </div>
              <button
            ...
            <div className="grid grid-cols-4 gap-3 text-xs">
              ...
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                <div className="text-slate-500">Unit Cost &bull; Primary Bin</div>
                <div className="text-base font-bold font-mono text-slate-800">₹{selectedItemForLots.unitCostInr} &bull; {selectedItemForLots.primaryBin}</div>
              </div>
            </div>
            ...
            {/* SECTION 2: Active Traceable Lots & Provenance Lineage */}
              ...
              {selectedItemForLots.lots && selectedItemForLots.lots.length > 0 ? (
                <div className="space-y-3">
                  {selectedItemForLots.lots.map((lot) => (
                    <div key={lot.lotNumber} className="border border-slate-200 rounded-xl p-3.5 bg-slate-50/50 space-y-3">
                      {/* Lot Header */}
                      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200/80 pb-2">
                        <div className="flex items-center gap-2">
                          <span className="font-mono font-bold text-xs text-[#0F8B8D] px-2 py-0.5 bg-teal-50 rounded border border-teal-200">
                            {lot.lotNumber}
                          </span>
                          <span className="text-xs font-semibold text-slate-700">
                            Batch: <span className="font-mono">{lot.supplierBatchNumber}</span>
                          </span>
                          <WarehouseStatusBadge status={lot.status} size="xs" />
                        </div>
                        <div className="flex items-center gap-3 text-xs font-mono">
                          <span className="text-slate-500">Bin: <strong className="text-slate-800">{lot.storageBin}</strong></span>
                          <span className="text-slate-500">Avail: <strong className="text-emerald-700">{lot.availableQuantityKg.toLocaleString()} {lot.uom || 'KG'}</strong></span>
                        </div>
                      </div>

                      {/* Origin ("Where From In") & Outward ("What Purpose Is Out") Cards */}
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                        {/* Where From In Card */}
                        <div className="p-2.5 bg-emerald-50/40 border border-emerald-200 rounded-lg space-y-1">
                          ...
                          <div className="text-slate-700">
                            <strong>Origin Source:</strong> {lot.inwardSource || `${lot.supplierName} (GRN Receipt)`}
                          </div>
                          <div className="text-slate-600 text-[11px]">
                            <strong>Dock / Bay:</strong> {lot.inwardOriginLocation || 'Inward Unloading Dock #1'} &bull; <strong>Ref:</strong> {lot.inwardDocumentRef || lot.grnReference}
                          </div>
                          <div className="text-slate-500 text-[10px]">
                            Received By: {lot.inwardReceivedBy || 'Ramesh K. (Inward Gate)'} &bull; Date: {lot.receiptDate}
                          </div>
                        </div>

                        {/* What Purpose Is Out Card */}
                        <div className="p-2.5 bg-orange-50/40 border border-orange-200 rounded-lg space-y-1">
                          <div className="flex items-center gap-1.5 font-bold text-orange-900 text-[11px]">
                            <ArrowUpRight className="w-3.5 h-3.5 text-[#E8622C]" />
                            What Purpose Is Out (Allocation &amp; Issue)
                          </div>
                          <div className="text-slate-700">
                            <strong>Purpose:</strong> {lot.outwardPurpose || (lot.allocatedQuantityKg > 0 ? `Issued ${lot.allocatedQuantityKg} for Scheduled Work Orders` : 'Available for Requisition')}
                          </div>
                          <div className="text-slate-600 text-[11px]">
                            <strong>Target Destination:</strong> {lot.outwardDestination || 'Shopfloor Staging'} &bull; <strong>Ref:</strong> {lot.outwardReference || 'WO-REQUISITION'}
                          </div>
                          <div className="text-slate-500 text-[10px]">
